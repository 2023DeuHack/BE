...
class ArticleView(APIView):
    def get(self, request, **kwargs):
        if kwargs.get('article_id') is None:
            ...
            article_serializer = ArticleSerializer(article_object, many=True)
            return Response(article_serializer.data, status=status.HTTP_200_OK)

        article_id = kwargs.get('article_id')
        article_object = Article.objects.get(id=article_id)
        article_serializer = ArticleSerializer(article_object, many=False)
        return Response(article_serializer.data, status=status.HTTP_200_OK)

    def post(self, request, format=None):
        article_serializer = ArticleSerializer(data = request.data, context={'request':request})
        if article_serializer.is_valid(writer=self.request.user):
